...
public class JdbcOutputFormat<V> extends DBOutputFormat<DbRecordWritable, V>
    ...
    public RecordWriter getHiveRecordWriter(JobConf jobConf, Path finalOutPath, Class<? extends Writable> valueClass, boolean isCompressed, Properties tableProperties, Progressable progress)
        ...
        TaskAttemptContext taskContext = ShimLoader.getHadoopShims().newTaskAttemptContext(
                jobConf, null);
        org.apache.hadoop.mapreduce.lib.db.DBOutputFormat delegate = new org.apache.hadoop.mapreduce.lib.db.DBOutputFormat();
        org.apache.hadoop.mapreduce.RecordWriter recordWriter = delegate.getRecordWriter(taskContext);
